...
export default function DetectionInputView({ onNavigate, onInitiateDetection }: DetectionInputViewProps) {
  ...
  const handleStartScan = (e: React.FormEvent) => {
    e.preventDefault();
    if (!fullName.trim() || !userId.trim() || !whatsapp.trim()) {
        ...
        <section className="col-span-7 bg-white border border-[#D4C9B0] shadow-sm p-6 flex flex-col justify-between">
          <form onSubmit={handleStartScan} className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-xs font-bold uppercase tracking-wider text-[#0c132c]">Kode ID</label>
                <div className="relative flex items-center border border-[#D4C9B0] px-3.5 py-2.5 bg-white">
                  <input
                    type="text"
                    value={userId}
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold uppercase tracking-wider text-[#0c132c]">Nomor WhatsApp</label>
                <div className="relative flex items-center border border-[#D4C9B0] px-3.5 py-2.5 bg-white">
                  <input
                    type="tel"
                    value={whatsapp}
                    ...
                    className="w-full bg-transparent text-xs text-[#0c132c] font-medium placeholder:text-[#76767e] outline-none"
                  />
                  <Phone className="w-4 h-4 text-[#76767e]" />
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2 pt-2">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <button
                  ...
                  className={`border flex items-center justify-between p-3 cursor-pointer select-none transition-all ${gender === 'Male'
                ...
                <button
                  ...
                  className={`border flex items-center justify-between p-3 cursor-pointer select-none transition-all ${gender === 'Female'
                      ? 'bg-white border-2 border-[#0c132c] shadow-sm'
                      : 'bg-white border-[#D4C9B0] text-[#76767e] hover:bg-gray-50'
                    }`}
                >
            ...
            {gender === 'Female' && (
              <div className="p-4 bg-[#E8DEC8]/30 border border-[#D4C9B0] flex items-center justify-between transition-all duration-300">
                ...
                <div
                  ...
                  className={`w-12 h-6 flex items-center rounded-full p-1 cursor-pointer transition-all duration-200 ${hasHijab ? 'bg-[#0c132c]' : 'bg-[#76767e]'
                    }`}
                >
                  <div className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-all duration-200 ${hasHijab ? 'translate-x-6' : 'translate-x-0'
                    }`} />
                </div>
              </div>
            )}
